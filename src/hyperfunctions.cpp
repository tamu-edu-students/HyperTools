...
void SpecSimilChild(int threadId, int algorithmId, int columnIndex, vector<Mat> *mlt, vector<int> *reference_spectrum_ptr, Mat *outputSimilarityImage)
{
    ...
    for (int rowIndex = 0; rowIndex < hyperspectralImage[1].rows; rowIndex++)
    {
        ...
        switch (algorithmId)
        { // Manipulation of similarity values not complete yet...
        case 0:
        ...
        case 7:
            ...
            break;
        case 8: // Testing NS3
            similarityValue = 255 * sqrt(pow(sqrt(1 / reference_spectrum.size()) * calculateEUD(reference_spectrum, pixel_spectrum), 2) + pow(1 - cos(calculateSAM(reference_spectrum, pixel_spectrum)), 2));
            break;
